  ...
  limx_arm_msgs__msg__JointData__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    limx_arm_msgs__msg__JointData * data =
      (limx_arm_msgs__msg__JointData *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!limx_arm_msgs__msg__JointData__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          limx_arm_msgs__msg__JointData__fini(&data[i]);
        }
        free(data);
        return false;
      }
