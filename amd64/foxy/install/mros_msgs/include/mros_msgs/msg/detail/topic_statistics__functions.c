  ...
  mros_msgs__msg__TopicStatistics__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mros_msgs__msg__TopicStatistics * data =
      (mros_msgs__msg__TopicStatistics *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mros_msgs__msg__TopicStatistics__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mros_msgs__msg__TopicStatistics__fini(&data[i]);
        }
        free(data);
        return false;
      }
