  ...
  std::ostream & out)
{
  ...
  {
    if (msg.parallel_solver_mode.size() == 0) {
      out << "parallel_solver_mode: []";
    } else {
      out << "parallel_solver_mode: [";
      size_t pending_items = msg.parallel_solver_mode.size();
      for (auto item : msg.parallel_solver_mode) {
        rosidl_generator_traits::value_to_yaml(item, out);
        if (--pending_items > 0) {
          out << ", ";
        }
...
inline std::string to_yaml(const JointCmdLimx & msg, bool use_flow_style = false)
{
...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, controller_msgs::msg::JointCmdLimx>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    ...
    std::forward<T>(msg).kp,
    std::forward<T>(msg).kd,
    std::forward<T>(msg).mode,
    std::forward<T>(msg).na,
    std::forward<T>(msg).parallel_solver_mode);
}
...
template<>
constexpr const char * data_type<controller_msgs::msg::JointCmdLimx>()
{
  return "controller_msgs::msg::JointCmdLimx";
}

template<>
constexpr const char * name<controller_msgs::msg::JointCmdLimx>()
{
  return "controller_msgs/msg/JointCmdLimx";
}
...
template<>
struct MessageTraits<controller_msgs::msg::JointCmdLimx>
{
