...
bool hand_msgs__msg__tactile_hand_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("hand_msgs.msg._tactile_hand_state.TactileHandState", full_classname_dest, 50) == 0);
  }
  hand_msgs__msg__TactileHandState * ros_message = _ros_message;
  {  // header
  ...
  {  // hand_tactile_state
    PyObject * field = PyObject_GetAttrString(_pymsg, "hand_tactile_state");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'hand_tactile_state'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    Py_ssize_t size = 2;
    hand_msgs__msg__TactileState * dest = ros_message->hand_tactile_state;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!hand_msgs__msg__tactile_state__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of TactileHandState */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("hand_msgs.msg._tactile_hand_state");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "TactileHandState");
  ...
  {  // hand_tactile_state
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->hand_tactile_state[i]);
      PyObject * pyitem = hand_msgs__msg__tactile_state__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    assert(PySequence_Check(field));
    {
      int rc = PyObject_SetAttrString(_pymessage, "hand_tactile_state", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
