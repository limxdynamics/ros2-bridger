...
class JointData(metaclass=Metaclass_JointData):
    ...
    def __init__(self, **kwargs):
        ...
        if 'angle' not in kwargs:
            self.angle = numpy.zeros(6, dtype=numpy.float32)
        else:
            self.angle = kwargs.get('angle')
        if 'velocity' not in kwargs:
            self.velocity = numpy.zeros(6, dtype=numpy.float32)
        else:
    ...
    def angle(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 6, \
                "The 'angle' numpy.ndarray() must have a size of 6"
            self._angle = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'angle' field must be a set or sequence with length 6 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
    ...
    def velocity(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 6, \
                "The 'velocity' numpy.ndarray() must have a size of 6"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -3.402823466e+38 or val > 3.402823466e+38) or math.isinf(val) for val in value)), \
                "The 'velocity' field must be a set or sequence with length 6 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
