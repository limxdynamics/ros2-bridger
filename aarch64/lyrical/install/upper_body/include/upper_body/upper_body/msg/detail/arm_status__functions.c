...
upper_body__msg__arm_status__create(void)
{
...
upper_body__msg__arm_status__Sequence__init(upper_body__msg__arm_status__Sequence * array, size_t size)
{
  ...
  if (size) {
    if (size > SIZE_MAX / sizeof(upper_body__msg__arm_status)) {
      return false;
    }
  ...
  upper_body__msg__arm_status__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    if (input->size > SIZE_MAX / sizeof(upper_body__msg__arm_status)) {
      return false;
    }
    ...
    upper_body__msg__arm_status * data =
      (upper_body__msg__arm_status *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!upper_body__msg__arm_status__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          upper_body__msg__arm_status__fini(&output->data[i]);
        }
        return false;
      }
    }
