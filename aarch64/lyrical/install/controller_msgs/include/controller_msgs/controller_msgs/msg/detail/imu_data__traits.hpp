...
// IWYU pragma: private, include "controller_msgs/msg/imu_data.hpp"
...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, controller_msgs::msg::IMUData>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    std::forward<T>(msg).header,
    std::forward<T>(msg).imustamp,
    std::forward<T>(msg).status,
    std::forward<T>(msg).euler,
    std::forward<T>(msg).quat,
    std::forward<T>(msg).acc,
    std::forward<T>(msg).gyro);
}
...
template<>
constexpr const char * data_type<controller_msgs::msg::IMUData>()
{
  return "controller_msgs::msg::IMUData";
}

template<>
constexpr const char * name<controller_msgs::msg::IMUData>()
{
  return "controller_msgs/msg/IMUData";
}
...
template<>
struct MessageTraits<controller_msgs::msg::IMUData>
{
