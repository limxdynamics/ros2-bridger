...
teleop_msgs__msg__TeleopLimbState__Sequence__init(teleop_msgs__msg__TeleopLimbState__Sequence * array, size_t size)
{
  ...
  if (size) {
    if (size > SIZE_MAX / sizeof(teleop_msgs__msg__TeleopLimbState)) {
      return false;
    }
  ...
  teleop_msgs__msg__TeleopLimbState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    if (input->size > SIZE_MAX / sizeof(teleop_msgs__msg__TeleopLimbState)) {
      return false;
    }
    ...
    teleop_msgs__msg__TeleopLimbState * data =
      (teleop_msgs__msg__TeleopLimbState *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!teleop_msgs__msg__TeleopLimbState__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          teleop_msgs__msg__TeleopLimbState__fini(&output->data[i]);
        }
        return false;
      }
    }
