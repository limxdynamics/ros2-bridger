  ...
  hand_msgs__msg__TactileHandState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    hand_msgs__msg__TactileHandState * data =
      (hand_msgs__msg__TactileHandState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hand_msgs__msg__TactileHandState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          hand_msgs__msg__TactileHandState__fini(&data[i]);
        }
        free(data);
        return false;
      }
