  ...
  controller_msgs__msg__JointCmdLimx__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    controller_msgs__msg__JointCmdLimx * data =
      (controller_msgs__msg__JointCmdLimx *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!controller_msgs__msg__JointCmdLimx__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          controller_msgs__msg__JointCmdLimx__fini(&data[i]);
        }
        free(data);
        return false;
      }
