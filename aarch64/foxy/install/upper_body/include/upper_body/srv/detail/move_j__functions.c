  ...
  upper_body__srv__moveJ_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    upper_body__srv__moveJ_Request * data =
      (upper_body__srv__moveJ_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!upper_body__srv__moveJ_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          upper_body__srv__moveJ_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  upper_body__srv__moveJ_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    upper_body__srv__moveJ_Response * data =
      (upper_body__srv__moveJ_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!upper_body__srv__moveJ_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          upper_body__srv__moveJ_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
