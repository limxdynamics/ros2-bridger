  ...
  teleop_msgs__msg__TeleopRawInput__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    teleop_msgs__msg__TeleopRawInput * data =
      (teleop_msgs__msg__TeleopRawInput *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!teleop_msgs__msg__TeleopRawInput__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          teleop_msgs__msg__TeleopRawInput__fini(&data[i]);
        }
        free(data);
        return false;
      }
