  ...
  teleop_msgs__msg__TeleopTarget__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    teleop_msgs__msg__TeleopTarget * data =
      (teleop_msgs__msg__TeleopTarget *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!teleop_msgs__msg__TeleopTarget__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          teleop_msgs__msg__TeleopTarget__fini(&data[i]);
        }
        free(data);
        return false;
      }
